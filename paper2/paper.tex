...
\usepackage{zi4} % 太字の\ttfamilyをサポートするフォント（予約語を\colorでのハイライトにする場合は不要）
...
%% （sumiilab-paper.clsの表紙は微妙にダサいですが、特に学部生は時間がないので気にせずそのまま使う方がいいかと。）
